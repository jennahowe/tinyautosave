...
(function() {
	...
	function setUserData(inst, str, exp) {
		...
		var ta = inst.editor.getElement(),
			s = getInstanceSettings(inst);
		
		ta.setAttribute(s.dataKey, str);
	...
	function getUserData(inst) {
		...
		ta.load("TinyMCE");
		return ta.getAttribute(getInstanceSettings(inst).dataKey);
	}
	...
	function removeUserData(inst) {
		...
		inst.editor.getElement().removeAttribute(getInstanceSettings(inst).dataKey);
	}
